...
class MotionPlusConstantsTests:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (MOTION_PLUS_ID_PREFIXES[0], bytes([0x00, 0x00, 0xA6, 0x20])),
            (MOTION_PLUS_ID_PREFIXES[1], bytes([0x01, 0x00, 0xA6, 0x20])),
        ],
    )
...
class TestOpcodeEnum:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (opcode.STATUS_REQUEST, 0x15),
            (opcode.WRITE_REGISTERS, 0x16),
            (opcode.READ_REGISTERS, 0x17),
            (opcode.SPEAKER_DATA, 0x18),
            (opcode.SPEAKER_MUTE, 0x19),
            (opcode.IR_CAMERA_ENABLE_2, 0x1A),
            # 入力レポート Opcodes
            (opcode.STATUS_INFORMATION, 0x20),
            (opcode.READ_DATA, 0x21),
            (opcode.ACK_REPORT, 0x22),
            # データレポート Opcodes
            (opcode.DATA_30, 0x30),
            (opcode.DATA_31, 0x31),
            (opcode.DATA_32, 0x32),
